...
-(id) init
{
	if ((self=[super init]) ) {
        ...
        self->resourceLabel = [CCLabelTTF labelWithString:@"Money $100" dimensions:CGSizeMake(150, 25) hAlignment:UITextAlignmentRight fontName:@"Marker Felt" fontSize:20];
        ...
        //Set up helth Bar
        CCSprite *healthSprite = [CCSprite spriteWithFile:@"health_bar_green.png"];
        healthBar = [CCProgressTimer progressWithSprite:healthSprite];
...
- (void)ccTouchMoved:(UITouch *)touch withEvent:(UIEvent *)event {  
	...
	if (selSprite) {
		...
		BOOL isBuildable = (bool)[((Tutorial *)m._gameLayer) canBuildOnTilePosition: touchLocationInGameLayer];
		if (isBuildable) {
...
- (void)ccTouchEnded:(UITouch *)touch withEvent:(UIEvent *)event {  
	...
	if (selSprite) {
		...
		if (!CGRectContainsPoint(backgroundRect, touchLocation)) {
			CGPoint touchLocationInGameLayer = [m._gameLayer convertTouchToNodeSpace:touch];
			[((Tutorial *)m._gameLayer) addTower: touchLocationInGameLayer: selSprite.tag];
		}
